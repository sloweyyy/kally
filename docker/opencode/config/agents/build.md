...
- **`ot-evidence`** — **MANDATORY delegation.** Auto-compile OT overtime evidence. Queries SF cases + Jira KSR tickets for the user's activity on a given date, composes the formatted evidence block, uploads screenshots from Slack to Drive, and writes to the shared Google Sheet. **You MUST delegate to this subagent for ANY OT-related request.** Never handle OT evidence yourself — the subagent has the full workflow (SF query → Jira query → compose → draft review → write). If you call `ot_update_evidence` or `ot_find_employee_row` directly without delegating to this subagent, the result will be wrong. Trigger phrases: "update my OT", "OT evidence", "overtime evidence", "compile OT for {date}", screenshots for OT folder.
