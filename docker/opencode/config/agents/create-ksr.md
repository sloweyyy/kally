...
Always call `getAccessibleAtlassianResources` once per session to resolve `cloudId` for `katalon.atlassian.net`, then cache it.
...
| Constant                          | Value                                 |
| --------------------------------- | ------------------------------------- |
| Cloud ID                          | `katalon.atlassian.net`               |
| Project key                       | `KSR`                                 |
...
```
https://katalon-inc.lightning.force.com/lightning/r/Case/{case.Id}/view
```

Always use the 18-char `Id` (e.g. `500RA00001HQlgvYAD`) — never the case number. This URL goes into `customfield_10504` as a string, not anywhere in the description body.
...
- `cloudId` = `katalon.atlassian.net` (or the UUID resolved from `getAccessibleAtlassianResources`)
- `projectKey` = `KSR`
...
```json
{
  ...
  "customfield_10504": "https://katalon-inc.lightning.force.com/lightning/r/Case/500RA00001HQlgvYAD/view",
...
| `getAccessibleAtlassianResources` fails       | Try `cloudId: "katalon.atlassian.net"` as a literal first; fall back to asking user                     |
