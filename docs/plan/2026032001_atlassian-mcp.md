...
Atlassian provides a hosted MCP server at `https://mcp.atlassian.com/v1/mcp` with Streamable HTTP transport. Auth is via Basic auth using a **scoped API token** (not a legacy token). This is identical to the Linear/PostHog pattern — no new container or custom server needed.
