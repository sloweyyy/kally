...
- ~~Should forced pushes (`forced === true`) use the same `git pull --ff-only` path and simply log failure if non-FF, or should they be skipped up front?~~ Resolved (D-8): force-sync via `fetch + reset --hard FETCH_HEAD` so force-pushes are reflected, not dropped.
