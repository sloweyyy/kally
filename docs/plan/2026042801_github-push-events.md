...
| D-8  | Replace `git pull --ff-only` with `git fetch origin refs/heads/<branch>` + `git reset --hard FETCH_HEAD` for non-delete sync                  | Force-pushes are deliberate human intent and should be reflected in the worktree, not silently dropped. The original `--ff-only` path swallowed external force-pushes (event dropped via `push_sync_failed`), leaving Thor's worktree stranded at a sha that no longer existed on the remote. Trade-off: `git reset --hard` discards uncommitted tracked-file edits in the worktree; acceptable because `coder`'s convention is to commit before relinquishing the worktree. Resolves Open Question 3. |
| D-9  | Skip the entire sync flow when `git rev-parse HEAD == event.after`                                                                            | Self-pushes and redelivered webhooks are common; if local HEAD already matches the pushed commit, fetching and resetting is wasted work and the wake would be a no-op. Pre-fetch rev-parse short-circuits with `push_sync_already_up_to_date`.                                                                                                                                                                                                                                                         |
| D-10 | Differentiate fast-forward from divergent reset using `git merge-base --is-ancestor HEAD FETCH_HEAD` and gate the wake `interrupt` flag on it | Fast-forwards add commits to a tip Thor was already building on; the agent can absorb them at the next yield. Divergent resets (force-push, rebase, branch rewrite) invalidate work in flight, so the wake must interrupt the current loop so the agent re-reads HEAD before continuing. Default-branch syncs follow the same detection but never wake — Thor does not run sessions on `main`.                                                                                                         |
| D-11 | Default-branch pushes sync the local repo but never enqueue a wake event                                                                      | Resolves Open Question 1. Thor does not run agent sessions on `main`/`master`; reaching the wake step there always produced `push_wake_skipped_no_session`. Returning the sync status directly removes the dead branch and clarifies operator intent.                                                                                                                                                                                                                                                  |
...
- ~~Should default branch pushes ever wake an existing `git:branch:<repo>:<default>` session, or should they only sync the repo?~~ Resolved (D-11): default-branch pushes sync only; no wake.
- ~~Should forced pushes (`forced === true`) use the same `git pull --ff-only` path and simply log failure if non-FF, or should they be skipped up front?~~ Resolved (D-8): force-sync via `fetch + reset --hard FETCH_HEAD` so force-pushes are reflected, not dropped. Further refined by D-10: divergent resets now wake the worktree session with `interrupt:true` so the agent re-reads HEAD instead of continuing on a sha that no longer exists.
...
2026-05-03 build: replaced `git pull --ff-only` with `git fetch + git reset --hard FETCH_HEAD` so force-pushes propagate to the worktree (D-8); updated 5 push-sync tests in `app.test.ts` to assert the new two-call sequence; updated `build.md` Reacting to PR events section accordingly.
2026-05-04 build: refined push handler with HEAD-equality short-circuit (D-9), fast-forward vs divergent-reset detection via `merge-base --is-ancestor` gating the wake `interrupt` flag (D-10), and default-branch never-wakes path (D-11); replaced `push_sync_*_pulled` statuses with `_fast_forwarded` / `_reset` variants and added `push_sync_already_up_to_date`; updated `app.test.ts` (68 tests pass) and aligned `docs/github-app-webhooks.md` and `build.md`.
