...
- [ ] Implement `packages/remote-cli/src/sandbox.ts` module:
  ...
  - Snapshot: launch from `daytona-medium` by default; override via `DAYTONA_SNAPSHOT` env var to use a custom snapshot built from `docker/sandbox/Dockerfile`. Image is published to `ghcr.io/scoutqa-dot-ai/thor-sandbox` by `.github/workflows/sandbox-image.yml`; an admin creates the Daytona snapshot from the published image. See `docs/daytona.md` for setup.
...
- [x] **P1 — Auto-stash dirty changes into sandbox.** Agent no longer needs `git commit` before every `sandbox` call. On dirty worktree: `git add -A` + temp commit (`thor-sandbox-wip`), sync, then `git reset --mixed HEAD~1` in finally block. If commit fails after staging, `git reset HEAD` restores original staging state. Reset failures are logged. Known limitation: delta sync falls back to full bundle after a dirty sync because the temp commit SHA is orphaned (P2 follow-up).
...
- [x] **P1 — Custom sandbox image with polyglot runtimes.** `docker/sandbox/Dockerfile` builds an Ubuntu 24.04 image with nvm (Node 20/22/24), SDKMAN (Java 17/21, Maven, Gradle), pyenv (Python 3.11/3.12/3.13), and uv. CI workflow (`.github/workflows/sandbox-image.yml`) builds and pushes the image to `ghcr.io/scoutqa-dot-ai/thor-sandbox` on Dockerfile changes; an admin creates the Daytona snapshot from the published image and sets `DAYTONA_SNAPSHOT` to use it. Default snapshot remains `daytona-medium` for zero-config startup.
