...
| 2026-04-17 | ~~Sandbox REQUIRES GitHub App auth, no PAT fallback~~                           | **Superseded by bundle sync (2026-04-18).** Sandbox no longer contacts origin — no auth needed. GitHub App auth remains for host-side git push only.                                                                                                                                                                                   |
| 2026-04-17 | Repo remote from `/workspace/repos/<repo>` not config.json                      | `/workspace/repos` is read-only to OpenCode, so `.git/config` there is effectively admin-owned. HTTPS enforced at resolution time. Still used for host-side git push — not needed for sandbox sync (bundle-based).                                                                                                                     |
...
| 2026-04-17 | ~~SHA-based identity for clone + sync; strict host preflight~~                  | **Superseded by bundle sync (2026-04-18).** SHA tracking remains (via `thor-sha` label) but the clone/fetch/push mechanism is replaced by bundle upload. Strict preflight (clean worktree) still applies.                                                                                                                              |
...
| 2026-04-17 | ~~Clone default branch then fetch+reset SHA~~                                   | **Superseded by bundle sync (2026-04-18).** No clone from origin at all — sandbox receives code via bundle upload.                                                                                                                                                                                                                     |
| 2026-04-17 | ~~Always scratch-push; fetch by ref, not by SHA~~                               | **Superseded by bundle sync (2026-04-18).** No pushes to origin for sandbox sync — bundle uploaded directly via SDK.                                                                                                                                                                                                                   |
| 2026-04-17 | ~~Embed GitHub App token in the sandbox fetch URL~~                             | **Superseded by bundle sync (2026-04-18).** Sandbox has no remotes and never fetches — no tokens needed anywhere in sandbox.                                                                                                                                                                                                           |
| 2026-04-17 | Auth delivery via GIT_ASKPASS callback (host-side git ops)                      | Host-side git commands (agent's `git push`) use `GIT_ASKPASS` → `bin/git-askpass` → `auth-helper.ts` to mint tokens on demand. Unchanged — this is independent of sandbox sync.                                                                                                                                                        |
