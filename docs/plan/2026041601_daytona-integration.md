...
2. **No writes to origin for sync.** Previous design pushed scratch refs (`refs/heads/kally-sandbox/*`) on every sync. Bundle upload eliminates all writes to the remote repo. No stale refs, no cleanup job.
...
- [ ] Implement `packages/remote-cli/src/sandbox.ts` module:
  ...
  - Snapshot: launch from `daytona-medium` by default; override via `DAYTONA_SNAPSHOT` env var to use a custom snapshot built from `docker/sandbox/Dockerfile`. Image is published to `ghcr.io/scoutqa-dot-ai/kally-sandbox` by `.github/workflows/sandbox-e2e.yml`; an admin creates the Daytona snapshot from the published image. See `docs/daytona.md` for setup.
...
- [x] **P1 — Auto-stash dirty changes into sandbox.** Agent no longer needs `git commit` before every `sandbox` call. On dirty worktree: `git add -A` + temp commit (`kally-sandbox-wip`), sync, then `git reset --mixed HEAD~1` in finally block. If commit fails after staging, `git reset HEAD` restores original staging state. Reset failures are logged. Known limitation: delta sync falls back to full bundle after a dirty sync because the temp commit SHA is orphaned (P2 follow-up).
- [x] **P1 — Real-time streaming via Daytona sessions.** Replaced buffered `executeCommand` with session-based streaming (`createSession` + `executeSessionCommand(runAsync)` + `getSessionCommandLogs(callbacks)`) so the agent sees build/test output as it happens instead of waiting for the command to finish. Session created per-exec and cleaned up in `finally` block. Also fixed `flushHeaders()` before async streaming so errors go through NDJSON path instead of producing invalid HTTP (Transfer-Encoding + Content-Length conflict).
...
- [x] **P1 — Custom sandbox image with polyglot runtimes.** `docker/sandbox/Dockerfile` builds an Ubuntu 24.04 image with nvm (Node 20/22), SDKMAN (Java 17/21, Maven, Gradle), pyenv (Python 3.12), and uv. CI workflow (`.github/workflows/sandbox-e2e.yml`) builds and pushes the image to `ghcr.io/scoutqa-dot-ai/kally-sandbox` on Dockerfile changes; an admin creates the Daytona snapshot from the published image and sets `DAYTONA_SNAPSHOT` to use it. Default snapshot remains `daytona-medium` for zero-config startup.
...
| 34  | Eng-v5 | Real-time streaming via Daytona sessions                 | Bug fix        | P1        | Buffered executeCommand blocks agent for minutes on long builds; session API streams via WebSocket                          | Buffered executeCommand             |
| 35  | Eng-v5 | Per-cwd promise-chaining mutex for parallel sandbox exec | Bug fix        | P1        | Two parallel calls raced on git auto-stash; lock wraps git+sync only, releases before streaming exec                        | Global mutex (blocks all cwds)      |
| 36  | Eng-v5 | flushHeaders before async streaming                      | Bug fix        | P1        | Without flush, streaming errors produced invalid HTTP (Transfer-Encoding + Content-Length)                                  | —                                   |
| 37  | Eng-v5 | Custom sandbox image published to GHCR                   | Mechanical     | P1        | Pre-installed runtimes (Node/Java/Python) eliminate apt-get on every sandbox create                                         | Generic `daytona-medium` snapshot   |
| 38  | Eng-v6 | Slim runtime contract + on-demand extras                 | Mechanical     | P1        | Keep base image focused (Node 22/20, Python 3.12, Java 21/17) and install less-common toolchains per task via helper script | Broad preinstall matrix             |
