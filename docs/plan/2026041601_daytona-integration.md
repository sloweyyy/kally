...
- [ ] Implement `packages/remote-cli/src/sandbox.ts` module:
  ...
  - Snapshot: launch from `thor-sandbox-base` by default (custom image with Node/Java/Python pre-installed); admin-overridable via `DAYTONA_SNAPSHOT` env var. Snapshot built from `docker/sandbox/Dockerfile` and published via `scripts/create-snapshot.ts` (CI: `.github/workflows/sandbox-image.yml`).
...
- [x] **P1 — Custom sandbox snapshot with polyglot runtimes.** `docker/sandbox/Dockerfile` builds an Ubuntu 24.04 image with nvm (Node 20/22/24), SDKMAN (Java 17/21, Maven, Gradle), pyenv (Python 3.11/3.12/3.13), and uv. `scripts/create-snapshot.ts` publishes it to Daytona as `thor-sandbox-base`. CI workflow (`.github/workflows/sandbox-image.yml`) auto-publishes on Dockerfile changes. Default snapshot changed from `daytona-medium` to `thor-sandbox-base`.
...
| 37  | Eng-v5 | Custom snapshot `thor-sandbox-base` from Dockerfile      | Mechanical     | P1        | Pre-installed runtimes (Node/Java/Python) eliminate apt-get on every sandbox create                  | Generic `daytona-medium` snapshot   |
