 ...
 *   endpoint: "git", "gh", "scoutqa", or "metabase"
 *
 * Env:
...
if (!endpoint) {
  process.stderr.write("Usage: remote-cli.mjs <git|gh|scoutqa|metabase> [args...]\n");
  process.exit(1);
}
...
try {
  const res = await fetch(url, {
    ...
    body: JSON.stringify(endpoint === "metabase" ? { args } : { args, cwd }),
  });
